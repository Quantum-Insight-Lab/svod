#!/usr/bin/env python3
"""Руки ступени R5: генерирует модуль render.py.

Чертёж-codegen для R5. render.py появляется только в фазе apply.
"""

from pathlib import Path

RENDER_PY = '''#!/usr/bin/env python3
...
target = Path(__file__).resolve().parent.parent / "render.py"
target.write_text(RENDER_PY, encoding="utf-8")
print(f"build_render: записан {target.name}")
